...
{
	printf("itemGetItemType called");

	CItemData * pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pItemData)
	{
		printf("itemGetItemType - No selected item data, returning exception");
		return Py_BuildException("no selected item data er");
	}

	printf("itemGetItemType - Selected item data found, item type: %d", pItemData->GetType());
