...
{
	std::string name = c_rkCreateData.m_stName;
	...
	if (IsEnemy())
	{
		auto& pythonSystem = CPythonSystem::Instance();
		auto& pythonNonPlayer = CPythonNonPlayer::Instance();

		#if defined(ENABLE_SHOW_MOBAIFLAG)
		bool isAggressive = pythonNonPlayer.IsAggressive(GetRace());

		if (pythonSystem.IsShowMobAIFlag() && isAggressive)
		{
			name += "*";
	...
	SetNameString(name.data(), nameLength);
}
