...
bool CPythonApplication::Process()
{
...
#endif
	//!@# Alt+Tab Áß SetTransfor żˇĽ­ Ć¨±č Çö»ó ÇŘ°áŔ» Ŕ§ÇŘ - [levites]
	//if (m_isActivateWnd)
	__UpdateCamera();
#ifdef __PERFORMANCE_CHECK__
	...
	//	//łĘą« ´ĘľúŔ» °ćżě µű¶óŔâ´Â´Ů.
	//	//±×¸®°í m_dwCurUpdateTime´Â deltaŔÎµĄ delta¶ű absolute timeŔĚ¶ű şń±łÇĎ¸é ľîÂĽŔÚ´Â°Ü?
	//	//if (dt >= 500 || m_dwCurUpdateTime > s_uiNextFrameTime)

	//	//±âÁ¸ÄÚµĺ´ë·Î ÇĎ¸é 0.5ĂĘ ŔĚÇĎ Â÷ŔĚł­ »óĹÂ·Î update°ˇ ÁöĽÓµÇ¸é °čĽÓ rendering frame skipąß»ý
	//	if (dt >= 500 || m_dwCurUpdateTime > s_uiNextFrameTime)
	//	{
	...
	if (s_bFrameSkip)
	{
	// ŔĚŔü ÇÁ·ąŔÓµµ ˝şĹµŔĚ¶ó¸é..
	if (s_isPrevFrameSkip)
	{
	if (s_dwFrameSkipEndTime==0)
	{
	s_dwFrameSkipCount=0; // ÇÁ·ąŔÓ ĂĽĹ©´Â ·Îµů ´ëşń
	s_dwFrameSkipEndTime=dwFrameSkipCurTime+ERROR_FRAME_SKIP_TIME; // ˝Ă°Ł ĂĽĹ©´Â ·ÎµůČÄ ÇÁ·ąŔÓ ˝şĹµ ĂĽĹ©
	...
	//#ifndef _DEBUG
	// ŔĎÁ¤ ˝Ă°ŁµżľČ °čĽÓ ÇÁ·ąŔÓ ˝şĹµ¸¸ ÇŃ´Ů¸é...
	...
	/*
	TraceError("ą«ÇŃ ÇÁ·ąŔÓ ˝şĹµŔ¸·Î Á˘ĽÓŔ» Áľ·áÇŐ´Ď´Ů");
	...
	*/
	if (!s_bFrameSkip)
	{
		...
		// ¸®˝şĹäľî Ăł¸®¶§¸¦ °í·ÁÇŘ ŔĎÁ¤ ˝Ă°ŁµżľČŔş ąöĆŰ¸µŔ» ÇĎÁö ľĘ´Â´Ů
		if (!canRender)
		{
			SkipRenderBuffering(3000);
		}
			...
			if (m_pyGraphic.Begin())
			{
				...
				if (dwCurFaceCount > 5000)
				{
					...
					{	
						...
						// ŔĎÁ¤ ÇÁ·ąŔÓ ĽÓµµżˇ ¸ÂĂßľîÁÖ´ÂÂĘżˇ ´«żˇ ĆíÇĎ´Ů
						// ľĆ·ˇżˇĽ­ ÇŃąř ÇĎ¸é ‰ç´?
