...
{
	const bool isAbsolute = (c_szFileName[1] == ':');
	bool packFirst = (m_iSearchMode == SEARCH_PACK_FIRST);

	auto checkPackAndFile = [&](auto getFirst, auto getSecond) -> bool {
		if (getFirst(rMappedFile, c_szFileName, pData))
		{
			return true;
		}

		if (!isAbsolute && getSecond(rMappedFile, c_szFileName, pData))
		{
#ifdef _DEBUG
			TraceError("%s", c_szFileName); // tylko do logowania, to nie jest b©©?d.
#endif
			return true;
		}
		return false;
	...
	if (packFirst)
	{
		return checkPackAndFile([this](auto&&... args) { return GetFromPack(args...); },
								[this](auto&&... args) { return GetFromFile(args...); });
	}
	else
	{
		return checkPackAndFile([this](auto&&... args) { return GetFromFile(args...); },
								[this](auto&&... args) { return GetFromPack(args...); });
	}
}
...
bool CEterPackManager::RegisterPack(const char* c_szName, const char* c_szDirectory, const BYTE* c_pbIV)
{
	...
	// SprawdŸ, czy paczka jest ju¿ zarejestrowana.
	if (m_PackMap.find(c_szName) != m_PackMap.end())
	{
		return true;
	}

	// Utwórz nowy obiekt CEterPack.
	CEterPack* pEterPack = new CEterPack;
	if (!pEterPack->Create(m_FileDict, c_szName, c_szDirectory, true, c_pbIV))
	{
#ifdef _DEBUG
	...
	// Dodaj paczkê do mapy paczek.
	m_PackMap.insert(TEterPackMap::value_type(c_szName, pEterPack));

	// Dodaj paczkê do listy paczek.
	m_PackList.push_front(pEterPack);

	// Jeœli podano katalog, dodaj paczkê do mapy paczek wed³ug katalogu.
