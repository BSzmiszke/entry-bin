...
void CActorInstance::SetShape(DWORD eShape, float fSpecular)
{
	...
	else
	{
		...
		else
		{
			...
			auto* lodModelThing = pRaceData->GetLODModelThing();
			bool canUseLOD = pModelThing && lodModelThing && pModelThing->GetTextureCount() == lodModelThing->GetTextureCount();

			for (std::size_t i = 0; canUseLOD && i < static_cast<std::size_t>(pModelThing->GetTextureCount()); ++i)
			{
				if (std::string_view(pModelThing->GetTexturePath(i)) != std::string_view(lodModelThing->GetTexturePath(i)))
				{
					canUseLOD = false;
				}
			}
			...
			if (canUseLOD)
			{
				RegisterLODThing(0, lodModelThing);
			}
