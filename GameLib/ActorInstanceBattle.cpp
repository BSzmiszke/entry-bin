...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	float invisibleTimeAdjustment = CalculateAdjustedInvisibilityDuration(uiSkill, c_rAttackData);
	rVictim.m_fInvisibleTime = CTimer::Instance().GetCurrentSecond() + invisibleTimeAdjustment;
...
float CActorInstance::CalculateAdjustedInvisibilityDuration(const uint32_t skillID, const NRaceData::TAttackData& attackData)
{
	static const int RACE_SHAMAN_WOMAN = 3, RACE_SHAMAN_MAN = 7;
	constexpr float ATTACK_SPEED_THRESHOLD_FOR_SHAMAN = 1.3f;
	constexpr float INVISIBILITY_DURATION_SCALING_FACTOR = 0.5f;
	constexpr float MINIMUM_INVISIBILITY_DURATION = 0.5f;
	constexpr float MAXIMUM_INVISIBILITY_DURATION = 5.0f;

	if ((GetRace() != RACE_SHAMAN_WOMAN && GetRace() != RACE_SHAMAN_MAN) || skillID != 0)
	{
		return 0.0f;
	}

	float attackSpeedScaling = (m_fAtkSpd - ATTACK_SPEED_THRESHOLD_FOR_SHAMAN) / ATTACK_SPEED_THRESHOLD_FOR_SHAMAN;
	attackSpeedScaling = max(0.0f, min(attackSpeedScaling, 1.0f));

	float adjustedInvisibilityDuration = attackData.fInvisibleTime * (1.0f - INVISIBILITY_DURATION_SCALING_FACTOR * attackSpeedScaling);
	adjustedInvisibilityDuration = max(MINIMUM_INVISIBILITY_DURATION, min(adjustedInvisibilityDuration, MAXIMUM_INVISIBILITY_DURATION));

	return adjustedInvisibilityDuration;
}
